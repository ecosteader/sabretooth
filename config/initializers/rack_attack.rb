...
class Rack::Attack
  ...
  throttle('throttle_api_media', limit: 900, period: 18.minutes) do |req|
    req.authenticated_user_id if req.post? && req.path.match?(%r{\A/api/v\d+/media\z}i)
  end

  throttle('throttle_media_proxy', limit: 980, period: 10.minutes) do |req|
  ...
  throttle('throttle_authenticated_paging', limit: 980, period: 10.minutes) do |req|
